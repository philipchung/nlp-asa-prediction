...
from models.interpret.shap_fasttext_model import ShapFastTextModelWrapper
from models.utils import resolve_paths_and_save_config
from omegaconf import DictConfig, OmegaConf
from src.modeling.datamodule import DataModule
from src.modeling.mlflow.fasttext_flavor import load_model
...
@hydra.main(
    config_path="../conf", config_name="fasttext_shap_config.yaml", version_base=None
)
def explain_fasttext_model(cfg: Union[dict, DictConfig]):
    ...
    shap_fasttext_model = ShapFastTextModelWrapper(fasttext_model=model)

    # # Get model output scores (used internally by shap.Explainer)
    # scores = shap_fasttext_model(text_list)

    # Define whitespace masker
    masker = shap.maskers.Text(r"\W")

    # Create SHAP Explainer
    explainer = shap.Explainer(
        model=shap_fasttext_model,
        masker=masker,
