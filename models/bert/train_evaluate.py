...
from models.bert.test import test_bert_model_with_mlflow
...
def train_evaluate_notebook(cfg: DictConfig = None):
    ...
    with hydra.initialize(config_path="../conf", version_base=None):
        ...
        train_and_evaluate_bert_model_with_mlflow(cfg)
        return cfg
...
def train_and_evaluate_bert_model_with_mlflow(
    ...
    Returns:
        dict with keys `model`, `datamodule`, `train_trainer`, `train_mlflow_run`,
        `model_info`, `train_tags`, `test_trainer`, `test_mlflow_run`, `test_tags`.
    """
    ...
    with mlflow.start_run(
        ...
        with mlflow.start_run(
            ...
            results = train_bert_model_with_mlflow(
                cfg, log_model=False, create_run=False
            )
            datamodule = results["datamodule"]
            model = results["model"]
            train_trainer = results["trainer"]
        ...
        with mlflow.start_run(
            ...
            test_results = test_bert_model_with_mlflow(
                cfg=cfg, model=model, datamodule=datamodule, create_run=False
            )
            model = test_results["model"]
            datamodule = test_results["datamodule"]
            test_trainer = test_results["trainer"]
